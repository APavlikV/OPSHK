...
def check_move(control, attack, chosen_defense):
    ...
    correct_answer = next(
        (
            ...
            if control == data["control"] and attack in data["attack_defense"]
        ),
        None,
    )
...
def generate_detailed_log(control, attack, chosen_defense, is_success):
    ...
    if is_success:
        ...
        log += f"✅ <b>УСПЕХ:</b> Полный блок! Контратака в <b>{counter_zone}</b>!"
    else:
        if control_success:
...
def generate_final_stats(correct_count, control_count, hint_count, total_steps):
    ...
    stats += f"✅ Полных блоков: <b>{correct_count}</b> из {total_steps}\n"
