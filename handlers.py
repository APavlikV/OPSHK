...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "rules":
        ...
        await query.edit_message_text(
            ...
            "Другие варианты контр атак так же много вариантные и нужно, вернее, полезно для понимания того, что последний элемент Аге уке не обязательно в ДЗ - возможен и уровень ТР\n\n"
            ...
            "<b>Во вторых</b>: как уже понятно в представленном описании нет третье части этих техник, поэтому может вызвать некоторое недопонимание.\n"
            "<b>Еще раз</b> ☝🏻 кждое техническое действие (аге, чото, учи и гедан) сотоит из 3 частей:\n"
            "<b>1)</b> защита от контроля\n"
            "<b>2)</b> контр атака\n"
            "<b>3)</b> завершающий элемент (это может быть как добивание так и вынужденная защита от атаки после контроля)\n\n",
            parse_mode="HTML"
